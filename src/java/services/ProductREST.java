...
/**
 * Provides the RESTful Web Service Methods
 *
...
public class ProductREST {
    ...
    @PersistenceContext(unitName = "CPD4414-BuildIt11-2016WPU")
    EntityManager em;
    ...
    @Path("{from}/{to}")
    @Produces("application/json")
    public Response getById(@PathParam("from") int from, @PathParam("to") int to) {
        Query q = em.createNamedQuery("Product.findFromTo");
        q.setParameter("from", from);
        q.setParameter("to", to);
        List<Product> subList = q.getResultList();
        JsonArrayBuilder json = Json.createArrayBuilder();
    ...
    public Response edit(JsonObject json) {
        ...
        try {
            ...
            Product p = (Product) em.createNamedQuery("Product.findByProductId")
                    .setParameter("productId", json.getInt("productId"))
                    .getSingleResult();
